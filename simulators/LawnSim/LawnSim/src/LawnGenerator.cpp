...
unique_ptr<Yard> LawnGenerator::Generate(GeoLocale locale, size_t rows, size_t cols) const 
{
   ...
   return unique_ptr<Yard>(new Yard(locale, GenerateCells(rows, cols, FillHeightsPerlin)));
}


bm::matrix<YardCell> LawnGenerator::GenerateCells(size_t rows, 
                                                  size_t cols, 
                                                  FillHeightFunc_t hFunc) const 
{
   bm::matrix<YardCell> cells(rows, cols, YardCell());
   ...
   for (unsigned int i = 0; i < rows; ++i) {
      for (unsigned int j = 0; j < cols; ++j) {
         cells(i,j) = YardCell(0, heights(i,j));
      }
   }
